...
import code.model.VehicleStatus;
...
//    @LoadBalanced
    ...
    /**
     * Enriches the provided {@link CurrentPosition} with the closest code.service location
     ...
     * <ul>
     *
     * @param currentPosition Will be enriched with the closest code.service location
     * @throws Exception
     */
//	@HystrixCommand(
...
//			fallbackMethod = "handleServiceLocationServiceFailure")
        ...
        switch (currentPosition.getVehicleStatus()) {
            ...
            case STOP_NOW:
                ResponseEntity<Resource<ServiceLocation>> result = this.restTemplate.exchange(
                        "http://code.service-location-code.service/serviceLocations/search/findFirstByLocationNear?location={lat},{long}",
    ...
    public void handleServiceLocationServiceFailure(CurrentPosition currentPosition) {
        LOGGER.error("Hystrix Fallback Method. Unable to retrieve code.service station info.");
    }
}
