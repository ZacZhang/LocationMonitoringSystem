        ...
        success: function (stompUrl) {
            if (stompUrl) {
                ...
                var on_error = function () {
                    /*
                     * Re-attempt to connect to code.web socket.
                     */
                    setTimeout(setupWebSocketConnection, 3000);
                };
                stompClient.connect('guest', 'guest', on_connect, on_error, '/');
            } else {
                console.error('Unknown URL for "code.service-location-updater". Vehicle location updates are disabled!');
            }
        },
        error: function (xhr, error) {
            console.error('Cannot retrieve "code.service-location-updater" URL.');
            console.error(JSON.stringify(error));
            /*
             * Re-attempt to setup a code.web socket connection.
             */
            setTimeout(setupWebSocketConnection, 3000);
        },
//	    dataType: 'json'
...
function initVehicles() {
    ...
    // processing - spin!
    $("#spinnerIcon").show();
    collectPages('/fleet-location-code.service/locations', function (result) {
...
function setupServiceCenters() {
    ...
    collectPages(
        '/code.service-location-code.service/serviceLocations',
        function (result) {
